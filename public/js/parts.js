...
(function () {
  ...
  /**
   * Подпись модели в фильтре.
   *
   * ⚠️ Внутри выбранной серии префикс серии ОТРЕЗАЕТСЯ (19.09.2026). Владелец:
   * «когда выбираю серию, выбираются все телефоны в серии, не могу выбрать
   * простой айфон отдельно, Pro или Pro Max». Ряд моделей при этом был на
   * месте и работал — но выглядел повтором ряда серий: там «iPhone 15 · 114»,
   * тут «iPhone 15 · 30», те же слова, и разницу видно только по числу.
   * Теперь внутри серии «iPhone 15» модели читаются как «Pro Max», «Pro»,
   * «Plus», «Обычный» — сразу видно, что это уточнение, а не то же самое.
   *
   * Случай «iPhone 15 серия» — это детали, подходящие ко всей серии
   * (проклейка и подобное), поэтому подпись «Общие для серии», а не пустая.
   */
  function подписьМодели(модель, серия) {
    var полная = устройствоИз(модель) === "MacBook" ? макбукМодель(модель) : String(модель || "");
    if (!серия) return полная;
    if (полная.toLowerCase().indexOf(серия.toLowerCase()) !== 0) return полная;
    var хвост = полная.slice(серия.length).trim();
    if (!хвост) return "Обычный";
    if (/^серия$/i.test(хвост)) return "Общие для серии";
    return хвост;
  }
  ...
  function перерисоватьФильтры() {
    ...
    ступень(ряды.модель, "модель", function (i) { return i.модель; }, {
      ...
      подпись: function (m) { return подписьМодели(m, filters.серия); },
