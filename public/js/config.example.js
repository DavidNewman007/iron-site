...
window.IRON_CONFIG = {
  ...
  /**
   * URL функции приёма оплаты. Пусто = кнопка «Оплатить онлайн» скрыта.
   * Пример: https://functions.yandexcloud.net/<id-функции-alfa-pay>
   *
   * Изменено 11.09.2026: раньше ключ назывался `yandexPayApiUrl` и вёл на
   * функцию Яндекс Пэй. Яндекс Пэй снят по решению владельца, эквайринг
   * переезжает в Альфа-Банк. Имя ключа сделано НЕЙТРАЛЬНЫМ намеренно: это уже
   * вторая смена провайдера, и клиентский код не должен знать, чей там шлюз —
   * он просто уходит навигацией на эту функцию со списком id товаров.
   *
   * Кнопки включаются ОДНИМ этим значением; пустое — кнопок нет нигде.
   *
   * ВКЛЮЧЕНО 15.09.2026 после контрольной оплаты на 2 ₽ по СБП: заказ прошёл,
   * статус в шлюзе 2 (списано), уведомление владельцу дошло, страница успеха
   * показала подтверждение. Функция alfa-pay работает на БОЕВОМ контуре
   * Альфа-Банка. Порядок и устройство — docs/ALFA-PAY-SETUP.md
