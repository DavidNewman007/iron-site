...
(function () {
  ...
  /**
   * Контрольная покупка: ?test=test-1rub кладёт в корзину виртуальный товар за
   * 1 ₽, которого нет в каталоге. Нужно, чтобы проверить всю цепочку — оплату,
   * уведомление, накладную СДЭК — не покупая настоящий телефон.
   *
   * Подменить цену этим нельзя: сумму всё равно считает облачная функция, а
   * товар с таким id она узнаёт по своей переменной PAY_TEST_ITEM_ID. Здесь
   * задаётся только то, что человек видит на экране.
   */
  (function положитьТестовыйТовар() {
    var id = new URLSearchParams(location.search).get("test") || "";
    if (!/^test-[a-z0-9-]{1,30}$/i.test(id)) return;
    try {
      localStorage.setItem(CART_KEY, JSON.stringify([
        { id: id, name: "Тестовый платёж IRON SERVICE", price: 1 },
      ]));
    } catch (e) { /* приватный режим — ничего не поделать */ }
  })();
